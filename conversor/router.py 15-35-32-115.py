...
def listar_suplementos():
    ...
    if termo:
        query = query.filter(
            (Suplemento.nome.ilike(f'%{termo}%')) |
            (Suplemento.marca.ilike(f'%{termo}%'))
        )

    if hasattr(Suplemento, ordenar):
        query = query.order_by(getattr(Suplemento, ordenar).desc())
    else:
        query = query.order_by(Suplemento.nome)

    suplementos = query.all()

    return render_template('listar_suplementos.html', suplementos=suplementos, termo=termo, ordenar=ordenar)
