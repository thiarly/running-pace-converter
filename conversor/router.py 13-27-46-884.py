...
@app.route('/editar_resumo/<int:id>', methods=['POST'])
@login_required
def editar_resumo(id):
    ...
    novo_nome = request.form.get("nome_treino", "").strip()
    novo_comentario = request.form.get("comentario", "").strip()

    if not novo_nome:
        flash("Nome do treino não pode estar vazio.", "danger")
        return redirect(url_for('resumo_view'))

    resumo.nome_treino = novo_nome
    resumo.comentario = novo_comentario
    database.session.commit()
